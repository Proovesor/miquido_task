...
export class User {
  ...
  static async verifyToken(token) {
    return await jwt.verify(token, "somesupersecret", (error, data) => {
...
export default class UserApi {
  static async signUp(req, res, next) {
    ...
    try {
      ...
      if (createdUser) {
        res
          .status(201)
          .json({ created: "User created succesfully", ...user.getInfo() });
      }
    } catch (error) {
  ...
  static async signIn(req, res, next) {
    ...
    try {
      ...
      res
        .status(200)
        .json({ token: user.signToken(), userInfo: user.getInfo() });
    } catch (error) {
      res.status(500).json({ error: error });
    }
  }
