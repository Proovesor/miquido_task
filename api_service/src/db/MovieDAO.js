...
export default class MovieDAO {
  ...
  static async handleFavoriteCheck(email, movie_id) {
    ...
    try {
      ...
      if (isFavorite) {
        await movies.updateOne({ _id: movieId }, { $pull: { fans: email } });
        return false;
      }
      await movies.updateOne({ _id: movieId }, { $push: { fans: email } });
      return true;
    } catch (error) {
      console.error(error);
    }
  ...
  static async averageRating(title) {
    ...
    try {
      const aggregationResult = await (
        await movies.aggregate([matchStage, projectionStage, averageCountStage])
      ).next();
  ...
  static async userPreferences(email, title) {
    try {
      ...
      const userRating = ratings.filter((rating) => rating.email === email);
